...
/**
 * Adapted from Matthew Horridge and Chris Mungall's OWL reasoner examples by Jie Zheng <br>
 * University of Pennsylvania <br>
 * Date: Nov-8-2011 <br>
 * 
 * See: http://geneontology.svn.sourceforge.net/viewvc/geneontology/OWLTools/src/owltools/OWLReasonerRunner.java?revision=3454&content-type=text%2Fplain
 */
public class OWLReasonerRunner {
	...
	public static void main(String[] args) {
		...
		// String ontStr = "C:/Documents and Settings/Jie/My Documents/Manuscript/2011_ontoDog/ontoDog/FGEDview/obi.owl";
	...
	public static OWLOntology getInferredSuperClasses (OWLOntologyManager manager, OWLOntology ont, String inferOntStr, OWLReasoner reasoner) {
	...
	public static OWLOntology getOntologyWithInferredSuperClasses (OWLOntologyManager manager, OWLOntology ont, String inferOntStr, OWLReasoner reasoner) {
	...
	public static OWLOntology getCleanedOntologyWithInferredSuperClasses (OWLOntologyManager manager, OWLOntology ont, String mergeOntStr, OWLReasoner reasoner) {
		...
		for (OWLClass cs: ont.getClassesInSignature()) {
			if (cs.toString().equals("Thing") || cs.toString().equals("Nothing")) 	continue;
		...
		for (OWLClass cs: ont.getClassesInSignature()) {
        	...
        	// If current class has inferred direct super class, check whether we need to remove its asserted super class 
        			...
        			for (OWLClass iCls: inferSupCls) {
        				if (supAxioms.contains(aCls.toString() + " " + iCls.toString())) {
        					...
        					if (!axioms.contains(ax)) {
        						axioms.add(ax);
	        				//	System.out.println("   " + OBIentity.getLabel (cs, ont, df) + " subclass of " + OBIentity.getLabel (aCls, ont, df));
        					}
        				}
        			}
        		}        	
