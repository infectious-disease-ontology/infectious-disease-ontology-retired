...
(defclass obo ()
  ((path :initarg :path :initform nil :accessor path)
   ...
   (line-no :initform 0 :accessor line-no)
   ))
...
(defmethod read-obo ((g obo))
  (with-open-file (f (path g))
    ...
    (setf (terms g)
	  (loop for line = (prog1 (read-line f  nil :eof) (incf (line-no g)))
	     until (eq line :eof)
	     for part = (caar (all-matches line "^\\[(.*?)\\]\s*" 1))
	     do  (assert (or part (equal line ""))  () "Line ~a: Didn't find a record! '~a'" (line-no g) line)
...
(defmethod read-obo-key-values ((g obo) stream)
  (loop for line = (prog1 (read-line stream nil nil) (incf (line-no g)))
